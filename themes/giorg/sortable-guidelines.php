<?php
/**
 * Template Name: Sortable Guidleines
 *
 * A custom page template with sortable guidelines.
 *
 */
...
<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
			<?php
			$post->shownotice = get_post_meta($post->ID, '_acg_gi_cpt_shownotice', true);
			?>
			
			<article>
			<h1 class="page-title<?php if($post->shownotice == "shownotice"){ echo ' noticepresent'; }?>">
				<?php 
				the_title();
				if($post->shownotice == "shownotice"){ 
						get_sidebar("notice"); 
				}
				?>				
			</h1>
			<div class="group columns one-column">
			<table class="border sorter">
			<?php
